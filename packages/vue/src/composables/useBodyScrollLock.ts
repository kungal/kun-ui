...
// Known limitations, both measured and deliberately left as they are:
//
...
// - An app that scrolls inside an element of its own (`#app { overflow: auto }`
//   under a full-height html/body) is not locked by any of this — nothing here
//   can discover that element. Base UI is handed the scroller for that reason.
...
// They are saved as LONGHANDS because CSSOM cannot serialize a shorthand out of
// one longhand: a page with an inline `body { overflow-y: auto }` read '' back
// from `style.overflow`, and unlock then blanked that longhand instead of
// putting it back.
const MANAGED_BODY = [
  'overflowX',
  'overflowY',
...
type SavedBody = Record<(typeof MANAGED_BODY)[number], string>
type SavedHtml = Record<(typeof MANAGED_HTML)[number], string>

let savedBody: SavedBody | null = null
let savedHtml: SavedHtml | null = null
...
const apply = (locked: boolean) => {
  ...
  if (locked) {
    ...
    if (savedBody) return
    savedBody = Object.fromEntries(
      MANAGED_BODY.map((k) => [k, body.style[k]])
    ) as SavedBody
    savedHtml = Object.fromEntries(
      MANAGED_HTML.map((k) => [k, html.style[k]])
    ) as SavedHtml
    ...
    // behind the overlay. It goes on <html> for the same reason the overflow
    // does — CSS Overscroll Behavior 1's own example sets it there, on "the
    // viewport defining element"; propagation from <body> is a UA convention
    // that specification never states.
  ...
  if (!savedBody) return
  for (const k of MANAGED_BODY) body.style[k] = savedBody[k]
  if (savedHtml) for (const k of MANAGED_HTML) html.style[k] = savedHtml[k]
