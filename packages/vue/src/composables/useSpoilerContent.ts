...
// Attaches click-to-reveal spoiler behaviour (+ code-block copy buttons) to a
// v-html prose container. DOM-level — operates on the rendered HTML, so it's
// framework-neutral. Used by KunContent; also usable standalone on a
// hand-rolled `<div ref class="kun-prose" v-html>`.
export const useSpoilerContent = (containerRef: Ref<HTMLElement | null>) => {
  ...
  const setupSpoilerAnimation = (spoilerEl: HTMLElement) => {
    ...
    const style = getComputedStyle(spoilerEl)
  ...
  const cleanup = () => {
    const container = containerRef.value
    if (!container) return
    spoilerAnimations.forEach((_, spoilerEl) => destroySpoilerAnimation(spoilerEl))
