...
//
// Why this file exists: `avatarFallbackPool` defaults to `[]`, and an empty pool
// makes `pickAvatarFallback` return the single bundled `KUN_AVATAR_FALLBACK`
// data URI for every seed. Nothing errors — the whole site just quietly renders
// one identical avatar for every user without an image, which is what shipped
// here after 2.32.0 moved the pool out of the library: the /components/avatar
// "sticker fallback" demo showed Alice, Bob and Carol as three copies of the
// same picture.
//
// This list is BUNDLED rather than fetched, which is the opposite of what
// docs/INTEGRATION.md §7.1 tells an SSR app to do, for one reason: this site is
// fully prerendered (`nitro.prerender.crawlLinks`). A value fetched at build time
// reaches the browser only through the payload, and the config is installed
// app-wide, so that payload rides on every page — /colors carried all 64 URLs
// without rendering a single avatar. Both versions were built and measured:
// fetch + useState put /components/avatar at 80797 B raw / 11922 B gzip, the
// bundled constant at 71494 B / 8567 B. 3.4 KB gzip per page, on 66 pages, to
// transfer a list the client already has.
//
// The list can be bundled safely only because these URLs are content-addressed
// (`<sha256>_128.webp`): unlike the positional `/stickers/{pack}/{n}.webp` path
// this API replaced, a snapshot of them cannot rot. New stickers not appearing
// in a component demo is not a defect worth a network round-trip per build.
//
// To refresh (never required, only if you want newer pictures in the demos):
//
//   curl -s https://sticker.kungal.com/api/v1/avatar-pool | jq -r '.data.urls[]'
//
// Keep all 64 rather than a sample: the pick is `hash(name) % pool.length`, so a
// shorter list re-assigns every user and every demo screenshot changes.
//
// Snapshot of pool version `cd41af02cb0050ea`.
export const KUN_AVATAR_POOL: string[] = [
